...
export function DnD({ children = null }: PropsWithChildren) {
  return (
    <div
      className={clsx(
        "border-slate-200 mdx-dnd flex flex-col gap-6 rounded-xl",
        "lg:flex-row",
...
export function ItemTag({ children = null }: PropsWithChildren) {
  return (
    <div
      className={clsx(
        "bg-violet-600/[0.08]  text-violet-600 inline-flex h-6 items-center gap-1 rounded-full px-2 text-[13px] font-medium",
        "dark:text-violet-400 dark:dark:bg-violet-400/10 dark:font-normal"
      )}
    >
      #{children}
    </div>
...
export function Items({
  ...
  return (
    <div className={clsx("flex flex-row gap-6", "md:gap-12 lg:gap-24")}>
      ...
      <div
        className={clsx(
          "border-slate-200 items-stretch border",
          "dark:border-slate-800"
        )}
      />
      <div
